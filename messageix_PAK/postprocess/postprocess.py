# -*- coding: utf-8 -*-
"""
Postprocessing helpers used before plotting.
"""
import pandas as pd
...
def multiply_df(df1, column1, df2, column2):
    """Merge df1 with df2 and multiply column1 by column2.

    Returns the merged dataframe with the product in column 'product'.
    """
...
def plotdf_com(msgSC, tec, com, direction, plotyrs, yr):
    """Aggregate commodities for plotting."""
...
def plotdf_sec(plotyrs, df, groupby, result, limit, lyr):
    ...
    for sec in list(sectors.keys()):
        df[dict_sectors['{}'.format(sec)]
           ] = df.filter(items=sectors['{}'.format(sec)]).sum(axis=1).to_frame()
...
def plotdf_ind(msgSC, tec, com, direction, plotyrs, yr):
    """Aggregate results for industry and energy sectors."""
