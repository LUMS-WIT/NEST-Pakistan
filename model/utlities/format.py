...
""" Run this file to generate provincial parameter .csv files for MESSAGE
"""
...
def format_demand(DATA):
    ...
    dmd.to_csv(DATA / 'demand.csv', index = False)
...
def format_hist_act(DATA):
    ...
    ppl = pd.read_excel(DATA / 'provincial_sources' / 'historical_activity_raw.xlsx',
                        sheet_name = 'powerplants',
                        usecols = cols,
                       ).dropna()
    
    therm = pd.read_excel(DATA / 'provincial_sources' / 'historical_activity_raw.xlsx',
                          sheet_name = 'thermal',
                          usecols = cols,
                         ).dropna()
    
    other_elec = pd.read_excel(DATA / 'provincial_sources' / 'historical_activity_raw.xlsx',
                               sheet_name = 'other_electricity',
                               usecols = cols,
                              ).dropna()
    
    prod = pd.read_excel(DATA / 'provincial_sources' / 'historical_activity_raw.xlsx',
    ...
    historical_activity = final.as_pandas()

    historical_activity = historical_activity[['region', 'variable', 'year', 'value', 'unit']]

    # Make historical activity dataframe
    hist_act = make_df('historical_activity',
                       node_loc = historical_activity['region'],
                       technology = historical_activity['variable'],
                       year_act = 2015, 
                       mode = 'M1',
                       time = 'year', 
                       value = historical_activity['value'], 
                       unit = 'GWa'
                      )
    
    hist_act.to_csv(DATA / 'historical_activity.csv', index = False)
...
def format_hist_cap(DATA):
    ...
    exports = pd.read_excel(DATA / 'provincial_sources' / 'historical_capacity_raw.xlsx',
                            sheet_name = 'electricity_exports',
                            usecols = ['region', 'technology',
                                       'year', 'value', 'unit'])

    powerplants = pd.read_excel(DATA / 'provincial_sources' / 'historical_capacity_raw.xlsx',
                                sheet_name = 'powerplants',
                                usecols = ['region', 'technology',
                                           'year', 'value', 'unit'])
    ...
    hist_cap.to_csv(DATA / 'historical_capacity.csv', index = False)
