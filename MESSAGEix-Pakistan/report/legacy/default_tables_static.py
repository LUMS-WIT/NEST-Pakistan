...
from . import postprocess, pp_utils
...
def retr_CO2emi(units_emi, units_ene_mdl):
    ...
    _Biogas_gases_h2_comb = _Biogas_tot * (
        pp.inp(
            ["h2_smr", "h2_smr_ccs", "h2_atr", "h2_atr_ccs", "h2_pyrolysis"],
            units_ene_mdl,
            inpfilter={"commodity": ["gas"]},
        )
        / _inp_all_gas_tecs
    ...
    vars["Difference|Stock|Activity|Natural Gas"] = pp.out(
...
def retr_SE_synfuels(units):
    ...
    +pp.out(
        "meth_h2",
        units,
        outfilter={"level": ["secondary"], "commodity": ["methanol"]},
    )
    ...
    vars["Hydrogen|Gas|w/o CCS"] = pp.out(
        ["h2_smr", "h2_atr", "h2_pyrolysis"],
        units,
        outfilter={"level": ["secondary"], "commodity": ["hydrogen"]},
    )
    vars["Hydrogen|Gas|w/o CCS|SMR"] = pp.out(
        ["h2_smr"], units, outfilter={"level": ["secondary"], "commodity": ["hydrogen"]}
    )
    vars["Hydrogen|Gas|w/o CCS|ATR"] = pp.out(
        ["h2_atr"], units, outfilter={"level": ["secondary"], "commodity": ["hydrogen"]}
    )
    vars["Hydrogen|Gas|w/o CCS|Pyrolysis"] = pp.out(
        ["h2_pyrolysis"],
        units,
        outfilter={"level": ["secondary"], "commodity": ["hydrogen"]},
    )
    ...
    vars["Hydrogen|Gas|w/ CCS|SMR"] = pp.out(
        ["h2_smr_ccs"],
        units,
        outfilter={"level": ["secondary"], "commodity": ["hydrogen"]},
    )
    vars["Hydrogen|Gas|w/ CCS|ATR"] = pp.out(
        ["h2_atr_ccs"],
        units,
        outfilter={"level": ["secondary"], "commodity": ["hydrogen"]},
    )
    ...
    vars["Hydrogen|Storage Losses"] = pp.inp(
        "h2_stor_geo", units, inpfilter={"commodity": ["hydrogen"]}
    )
...
def retr_SE_gases(units):
    ...
    vars["H2 Blend"] = pp.inp("h2_mix", units)
    vars["Hydrogen"] = pp.out(["gas_h2"], units)
...
def retr_SE_elecgen(units):
    ...
    _SolarPV_OR = pp.act_rel(
        ["solar_cv1", "solar_cv2", "solar_cv3", "solar_cv4"],
        relfilter={"relation": ["oper_res"]},
        units=units,
    )

    _Wind_OR = pp.act_rel(
        ["wind_cv1", "wind_cv2", "wind_cv3", "wind_cv4"],
        relfilter={"relation": ["oper_res"]},
        units=units,
    )
    ...
    _SolarPV_woStorage_total = _SolarPV_raw - _Storage_input * (
        _SolarPV_OR / (_Wind_OR + _SolarPV_OR)
    ).fillna(0)
    ...
    _Wind_woStorage = _Wind - (
        _Storage_input * _Wind_OR / (_Wind_OR + _SolarPV_OR)
    ).fillna(0)
    _Wind_woStorage_onshore = _Wind_woStorage * (_Wind_onshore / _Wind).fillna(0)
    _Wind_woStorage_offshore = _Wind_woStorage * (_Wind_offshore / _Wind).fillna(0)
    _Wind_wStorage = _Storage_output * (_Wind_OR / (_Wind_OR + _SolarPV_OR)).fillna(0)
    _Wind_wStorage_onshore = _Wind_wStorage * (_Wind_onshore / _Wind).fillna(0)
    _Wind_wStorage_offshore = _Wind_wStorage * (_Wind_offshore / _Wind).fillna(0)

    vars["Wind|Onshore"] = _Wind_onshore
    vars["Wind|Offshore"] = _Wind_offshore
    vars["Wind|Curtailment"] = _Wind_curt_inp
    vars["Wind|Onshore|w/o Storage"] = _Wind_woStorage_onshore
    vars["Wind|Offshore|w/o Storage"] = _Wind_woStorage_offshore
    vars["Wind|Onshore|w/ Storage"] = _Wind_wStorage_onshore
    vars["Wind|Offshore|w/ Storage"] = _Wind_wStorage_offshore
    ...
    vars["Other"] = (
        ...
        + pp.inp(
            "h2_gas_fc_trp",
            units,
            inpfilter={"level": ["final"], "commodity": ["electr"]},
        )
        + pp.inp(
...
def retr_pe(units, method=None):
    ...
    vars["Oil|w/o CCS"] = (
        ...
        + pp.out(["oil_imp", "foil_imp", "loil_imp"], units)
    ...
    _Gas_wCCS = (
        ...
        + pp.inp(
            ["gas_cc_ccs", "h2_smr_ccs", "h2_atr_ccs"],
            units,
            inpfilter={"commodity": ["gas"]},
        )
    )
...
def retr_ppl_capparameters(prmfunc, units):
    ...
    vars["Hydrogen|Gas|w/o CCS"] = prmfunc(["h2_smr", "h2_atr", "h2_pyrolysis"])
...
def retr_ppl_parameters(prmfunc, units):
    ...
    vars["Hydrogen|Gas|w/o CCS"] = prmfunc(
        ["h2_smr", "h2_atr", "h2_pyrolysis"], units=units
    )
...
def retr_fe(units):
    ...
    H2Ind = pp.inp(["h2_i", "h2_fc_I"], units)
    # Add blended hydrogen in industry
    H2INDBlend = _Hydrogen_ind
    ...
    vars["Industry|Hydrogen"] = H2Ind + H2INDBlend
    vars["Industry|Hydrogen|Direct"] = H2Ind
    vars["Industry|Hydrogen|Blend"] = H2INDBlend
    ...
    vars["Residential and Commercial|Hydrogen"] = H2rc + H2rc_blend
    vars["Residential and Commercial|Hydrogen|Direct"] = H2rc
    vars["Residential and Commercial|Hydrogen|Blend"] = H2rc_blend
    ...
    vars["Transportation|Liquids|Ethanol"] = EthTRP + EthTRP_shipping
    vars["Transportation|Liquids|Methanol|Shipping"] = EthTRP_shipping
    vars["Transportation|Liquids|Methanol"] = MethTRP + MethTRP_shipping
    vars["Transportation|Liquids|Methanol|Shipping"] = MethTRP_shipping
...
def retr_trade(units_energy, units_CPrc_co2, units_emi_val, units_emi_vol):
    ...
    # Calculate net trade
    _Biomass_total = pp.out("biomass_imp", units=units_energy) - pp.inp(
        "biomass_exp", units=units_energy
    )

    _Coal_total = pp.out("coal_imp", units=units_energy) - pp.inp(
        "coal_exp", units=units_energy
    )

    _CrudeOil_total = pp.out(["oil_imp"], units=units_energy) - pp.inp(
        ["oil_exp"], units=units_energy
    )

    _Oilprod_total = pp.out(["foil_imp", "loil_imp"], units=units_energy) - pp.inp(
        ["foil_exp", "loil_exp"], units=units_energy
    )

    _Gas_total = pp.out(["LNG_imp", "gas_imp"], units=units_energy) - pp.inp(
        [
            "gas_exp" "LNG_exp",
        ],
        units=units_energy,
    ...
    _H2_total = pp.out("lh2_imp", units=units_energy) - pp.inp(
        "lh2_exp", units=units_energy
    )

    _BTL_total = pp.out("eth_imp", units=units_energy) - pp.inp(
        "eth_exp", units=units_energy
    )

    _FTL_total = pp.out("meth_imp", units=units_energy) - pp.inp(
    ...
    vars["Gross Export|Secondary Energy|Hydrogen|Volume"] = pp.out(
        ["lh2_exp"], units=units_energy
    )

    vars["Gross Import|Primary Energy|Biomass|Volume"] = pp.out(
        "biomass_imp", units=units_energy
    )

    vars["Gross Import|Primary Energy|Coal|Volume"] = pp.out(
        "coal_imp", units=units_energy
    )

    vars["Gross Import|Primary Energy|Oil|Volume"] = pp.out(
        ["oil_imp"], units=units_energy
    )

    vars["Gross Import|Primary Energy|Gas|Volume"] = pp.out(
        ["LNG_imp", "gas_imp"], units=units_energy
    )

    vars["Gross Import|Secondary Energy|Electricity|Volume"] = pp.out(
        [
            "elec_imp",
            # "elec_imp_africa",
            # "elec_imp_america",
    ...
    vars["Gross Import|Secondary Energy|Liquids|Biomass|Volume"] = pp.out(
        "eth_imp", units=units_energy
    )

    vars["Gross Import|Secondary Energy|Liquids|Coal|Volume"] = pp.out(
        ["meth_imp"], units=units_energy
    )

    vars["Gross Import|Secondary Energy|Liquids|Oil|Volume"] = pp.out(
        ["foil_imp", "loil_imp"], units=units_energy
    )
    vars["Gross Import|Secondary Energy|Hydrogen|Volume"] = pp.out(
        ["lh2_imp"], units=units_energy
    )
...
def retr_supply_inv(units_energy, units_emi, units_ene_mdl):
    ...
    _Gas_synf_ccs_h2 = (
        pp.investment(["h2_smr_ccs", "h2_atr_ccs"], units=units_energy) * 0.17
    )
    ...
    vars["Hydrogen|Fossil"] = (
        pp.investment(
            ["h2_coal", "h2_smr", "h2_atr", "h2_pyrolysis"], units=units_energy
        )
    ...
    vars["Hydrogen|Fossil|Coal"] = (
        pp.investment(["h2_coal"], units=units_energy)
        + pp.investment("h2_coal_ccs", units=units_energy) * 0.97
        + _Coal_synf_ccs_h2
    )
    vars["Hydrogen|Fossil|Gas|SMR"] = (
        pp.investment(["h2_smr"], units=units_energy)
        + pp.investment(["h2_smr_ccs"], units=units_energy) * 0.83
        + _Gas_synf_ccs_h2
    )
    vars["Hydrogen|Fossil|Gas|ATR"] = (
        pp.investment(["h2_atr"], units=units_energy)
        + pp.investment(["h2_atr_ccs"], units=units_energy) * 0.83
    )
    vars["Hydrogen|Fossil|Gas|Pyrolosis"] = pp.investment(
        ["h2_pyrolysis"], units=units_energy
    )

    vars["Hydrogen|Renewable|Biomass"] = (
    ...
    vars["Hydrogen|Other|Electrolyzer"] = pp.investment(
        [
            "h2_elec",
        ],
        units=units_energy,
    )
    vars["Hydrogen|Other|Blend"] = pp.act_vom("h2_mix", units=units_energy) * 0.5
    vars["Hydrogen|Other|Liquefaction"] = pp.investment(
        [
            "h2_liq",
        ],
        units=units_energy,
    )
    vars["Hydrogen|Other|Infrastructure"] = pp.investment(
        [
            "h2_t_d",
            "lh2_exp",
            "lh2_imp",
...
def retr_water_use(units, method):
    ...
    _Direct_ele_gas_woCCS = (
        ...
        + _out_div_eff(
            ["h2_smr", "h2_atr", "h2_pyrolysis"], group, inpfilter, outfilter
        )
    )
    ...
    vars["Hydrogen|Gas|w/o CCS"] = _out_div_eff(
        ["h2_smr", "h2_atr", "h2_pyrolysis"], group, inpfilter, hydrogen_outputfilter
    )
    ...
    if method == "consumption":
        ...
        vars["Hydrogen|Gas|w/o CCS"] -= pp.act_emif(
            ["h2_smr", "h2_atr", "h2_pyrolysis"], units=units, emiffilter=emiffilter
        )
