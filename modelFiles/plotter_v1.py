...
def add_history(msgBAS, tecs, nodeloc, df2, groupby):
    df1_hist = msgBAS.par('historical_activity',
...
def model_output(msgBAS, tecs, nodeloc, parname, coms=None):
    df1 = msgBAS.var('ACT', {'technology': tecs, 'node_loc': nodeloc})
    df2 = msgBAS.par(parname, {'technology': tecs, 'node_loc': nodeloc})
    if coms:
...
def plotter(caseName, msgBAS, path, yr_min, yr_max):
    ...
    last_yr = msgBAS.set('cat_year', {'type_year': 'lastmodelyear'}
                        )['year'].item()
    plotyrs = [int(i) for i in list(msgBAS.set('year')) if
               int(i) < int(last_yr)]
    yr = msgBAS.set('cat_year', {'type_year': 'firstmodelyear'}).year.item()
    ...
    cap = msgBAS.var('CAP', {'year_vtg': plotyrs})
    cap_new = msgBAS.var('CAP_NEW', {'year_vtg': plotyrs})
    cap_hist = msgBAS.par('historical_new_capacity', {'year_vtg': plotyrs})

    tec = msgBAS.par('output', {'commodity': 'electr', 'level': 'secondary'}
                    )['technology']
    ...
    elec_out_plot = plotdf(msgBAS, tec, ['electr'], 'output', plotyrs, yr
                           ) * unit_el

    # Treatment of storage losses
#    d_stor = msgBAS.par('input', {'technology': 'stor_ppl'})
...
#    elec_out_plot.rename({'stor_ppl':'storage_loss'}, axis=1, inplace=True)
    ...
    tecs = list(set(msgBAS.par('input', {'commodity': 'electr',
                                        'level': 'final'}).technology))
    tecs = tecs + ['stor_ppl']
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input', 'electr')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)

    # adding historical data
    df_hist = add_history(msgBAS, tecs, nodeloc, df2, 'technology')
    ...
    tecs = list(set(msgBAS.par('output', {'commodity': 'gas'}
                              ).technology) - set(gas_tecs))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output', 'gas')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    ...
    # Natural gas usage
    gas_tecs = ['gas_t_d', 'gas_t_d_ch4', 'gas_bal']
    tecs = list(set(msgBAS.par('input', {'commodity': 'gas'}
                              ).technology) - set(gas_tecs))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input', 'gas')
    df = plotdf_sec(plotyrs, df, ['year_act', 'technology'],
                    'product', 0.0, yr)

    # adding historical data
    # df_hist = add_history(msgBAS, tecs, nodeloc, df2)
    ...
    tecs = list(set(msgBAS.par('output', {'commodity': 'coal'}
                              ).technology) - set(coal_tecs))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output', 'coal')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('input', {'commodity': 'coal'}
                              ).technology) - set(coal_tecs))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input', 'coal')
    df = plotdf_sec(plotyrs, df, ['year_act', 'technology'],
                    'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('output', {'commodity': ['fueloil', 'lightoil'],
                                         'level': ['secondary']}).technology))

    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output',
                           ['fueloil', 'lightoil'])
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('input', {'commodity': ['fueloil', 'lightoil']}
                              ).technology) - set(['loil_t_d', 'foil_t_d']))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input',
                           ['fueloil', 'lightoil'])
    ...
    # 2.6) Oil supply
    tecs = list(set(msgBAS.par('output', {'commodity': ['crudeoil']}
                              ).technology) - set(['oil_bal', 'oil_exp']))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output', 'crudeoil')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('output', {'commodity': ['biomass'],
    ...
    # Biomass utilisation
    tecs = list(set(msgBAS.par('input',
                              {'commodity': ['biomass'],
                               'level': ['primary', 'final']}).technology
                    ) - set(['biomass_t_d']))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input', 'biomass')
    ...
    # 3.1) Transport
    tecs = list(set(msgBAS.par('output',
                              {'commodity': ['transport']}).technology))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)

    # adding historical data
    df_hist = add_history(msgBAS, tecs, nodeloc, df2, 'commodity')
    df = (df_hist + df).fillna(0)
    alldf['Transportation ' + unitname] = df * unitc

    # 3.2) Industry

    tecs = list(set(msgBAS.par('output', {'commodity': ['i_spec', 'i_therm']}
                              ).technology))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('output', {'commodity': ['i_feed']}).technology))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'input')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    ...
    tecs = list(set(msgBAS.par('output', {'level': 'primary'}).technology))
    ...
    tecs_exp = [x.split('_')[0] + '_exp' for x in tecs_imp if
                x.split('_')[0] + '_exp' in list(msgBAS.set('technology'))]

    df, df2 = model_output(msgBAS, tecs + tecs_imp, nodeloc, 'output')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    df = df.reindex(['crudeoil', 'biomass', 'electr', 'gas', 'lightoil',
                     'coal', 'fueloil', 'u5', 'rc_therm', 'others'], axis=1)

#    df_exp, df2_exp = model_output(msgBAS, tecs_exp, nodeloc, 'output')
...
#    df = (df_hist + df).fillna(0)
    ...
    tecs = list(set(msgBAS.par('output', {'level': ['useful']}).technology))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output')
    ...
    tecs = list(set(msgBAS.par('output', {'level': ['final']}).technology))
    df, df2 = model_output(msgBAS, tecs, nodeloc, 'output')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    ...
    # 5) Energy trade
    # 5.1) Exports
    df, df2 = model_output(msgBAS, tecs_exp, nodeloc, 'output')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    ...
    # 5.2) Imports
    df, df2 = model_output(msgBAS, tecs_imp, nodeloc, 'output')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    ...
    # 6) Emissions
    ems = ['TCE']
    df1 = msgBAS.var('EMISS', {'emission': ems, 'node': nodeloc})
    alldf['Total GHG emissions (MtCeq)'] = group(df1, ['year', 'emission'],
                                                 'lvl', 0.0, yr)
    # 7 ) Costs of commoditites
    # costs_useful = msgBAS.var('PRICE_COMMODITY', {'level':'useful'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_secondary = msgBAS.var('PRICE_COMMODITY', {'level':'secondary'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_primary = msgBAS.var('PRICE_COMMODITY', {'level':'primary'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_final = msgBAS.var('PRICE_COMMODITY', {'level':'final'})
