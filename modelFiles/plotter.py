...
# %% The main function
def plotter(caseName, msgSC, path, yr_min, yr_max):
...
#    df = (df_hist + df).fillna(0)
    ...
    # costs_useful = msgSC.var('PRICE_COMMODITY', {'level':'useful'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_secondary = msgSC.var('PRICE_COMMODITY', {'level':'secondary'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_primary = msgSC.var('PRICE_COMMODITY', {'level':'primary'})
    # alldf['Energy imports '+ unitname] = group(df, ['year_act', 'commodity'],'product',
    #                                  0.0, yr) * unitc
    
    # costs_final = msgSC.var('PRICE_COMMODITY', {'level':'final'})
