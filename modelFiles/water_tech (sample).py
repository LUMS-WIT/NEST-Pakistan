# -*- coding: utf-8 -*-
"""
This function add main parameters of water technologies (technical_lifetime, input, output, inv_cost, and var_cost)
...
def new_tech(msgSC, msgWSC, path_inputs):
    
    iso = msgSC.model.split('|')[0]
    reg = msgSC.model.split('|')[1]
    
    # region country mapping
    reg_df = pd.read_excel( str(path_inputs) + str('\\') + str('iso_reg.xlsx'), 'Sheet1')
    reg_df['node'] = str('R14_') + reg_df['node'].astype(str) 
    
    ### ONSHORE WIND
    
    # add renewable level
    msgSC.add_set('level','renewable')
    ...
    onshore_wind_costs_df = pd.read_excel(
            str(path_inputs) + str('\\') + str('irena_renewables_cost_2018.xlsx'), 
            'onshore_wind', 
            header = 0,
    ...
    if not df.empty:
        ...
        df = onshore_wind_costs_df.loc[onshore_wind_costs_df['R14'].isin([reg])]
        if not df.empty:
    ...
    if not df.empty:
        ...
        df = solar_pv_costs_df.loc[solar_pv_costs_df['R14'].isin([reg])]
        if not df.empty:
    ...
    df_hydro_pot = pd.read_excel(
        str(path_inputs) + str('\\') + str('image_hydro_cost_country.xlsx'), 
        str('MAX_POTENTIAL'))
