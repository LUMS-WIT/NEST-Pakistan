"""
Plotter for MESSAGEix-Pakistan model results.

Generates stacked bar charts saved to a PDF and an Excel workbook.
"""
import time
...
from .postprocess import plotdf_sec, plotdf, group, multiply_df
...
def plotter(msgSC, caseName, path):
    """Extract model results into a dict of DataFrames for plotting."""
    path = Path(path)

    nodeloc = msgSC.set('node')[1]
    writer_xls = pd.ExcelWriter(path / 'plots' / f'{caseName}.xlsx',
                                engine='xlsxwriter')
    ...
    unitc = 8.76 * 3.6       # GWa → PJ
    unitname = '(PJ)'
    unit_el = 8760 / 1000    # GWa → TWh
    unitname_el = '(TWh)'

    last_yr = msgSC.set('cat_year', {'type_year': 'lastmodelyear'})['year'].item()
    plotyrs = [int(i) for i in list(msgSC.set('year')) if int(i) < int(last_yr)]
    ...
    tec = msgSC.par('output', {'commodity': 'electr', 'level': 'secondary'})['technology']
    ...
    elec_out_plot = plotdf(msgSC, tec, ['electr'], 'output', plotyrs, yr) * unit_el
    ...
    tecs = list(set(msgSC.par('input', {'commodity': 'electr', 'level': 'final'}).technology))
    tecs = tecs + ['stor_ppl']
    ...
    # 2.3 Natural Gas supply and usage
    gas_tecs = ['gas_t_d', 'gas_t_d_ch4', 'gas_bal']
    tecs = list(set(msgSC.par('output', {'commodity': 'gas'}).technology) - set(gas_tecs))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'output', 'gas')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    df_hist = add_history(msgSC, tecs, nodeloc, df2, 'technology')
    alldf['Gas supply ' + unitname] = (df_hist + df).fillna(0) * unitc

    tecs = list(set(msgSC.par('input', {'commodity': 'gas'}).technology) - set(gas_tecs))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input', 'gas')
    df = plotdf_sec(plotyrs, df, ['year_act', 'technology'], 'product', 0.0, yr)
    alldf['Gas utilization ' + unitname] = df * unitc

    # 2.4 Coal supply and utilization
    coal_tecs = ['coal_t_d', 'coal_bal', 'coal_exp']
    tecs = list(set(msgSC.par('output', {'commodity': 'coal'}).technology) - set(coal_tecs))
    ...
    tecs = list(set(msgSC.par('input', {'commodity': 'coal'}).technology) - set(coal_tecs))
    ...
    df, df2 = model_output(msgSC, tecs, nodeloc, 'output', ['fueloil', 'lightoil'])
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    ...
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input', ['fueloil', 'lightoil'])
    alldf['Oil derivative use ' + unitname] = plotdf_sec(
        plotyrs, df, ['year_act', 'technology'], 'product', 0.0, yr) * unitc
    ...
    oil_extraction = ['oil_extr_1', 'oil_extr_2', 'oil_extr_3', 'oil_extr_4',
                      'oil_extr_5', 'oil_extr_6', 'oil_extr_7', 'oil_extr_1_ch4',
                      'oil_extr_2_ch4', 'oil_extr_3_ch4', 'oil_extr_4_ch4']
    df['oil_extraction'] = (df['oil_extr_1'] + df['oil_extr_2'] + df['oil_extr_3']
                            + df['oil_extr_4'] + df['oil_extr_5'] + df['oil_extr_6']
                            + df['oil_extr_7'])
    df = df.drop(columns=oil_extraction, axis=1)
    df_hist = add_history(msgSC, tecs, nodeloc, df2, 'technology')
    df_hist['oil_extraction'] = (df_hist['oil_extr_1'] + df_hist['oil_extr_2']
                                 + df_hist['oil_extr_3'] + df_hist['oil_extr_4']
                                 + df_hist['oil_extr_5'] + df_hist['oil_extr_6']
                                 + df_hist['oil_extr_7'])
    df_hist = df_hist.drop(columns=oil_extraction, axis=1)
    alldf['Oil supply ' + unitname] = (df + df_hist).fillna(0) * unitc

    # 2.7 Biomass provision and utilisation
    ...
    tecs = list(set(msgSC.par('input', {'commodity': ['biomass'],
                                        'level': ['primary', 'final']}).technology
                    ) - set(['biomass_t_d']))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input', 'biomass')
    alldf['Biomass utilization ' + unitname] = plotdf_sec(
        plotyrs, df, ['year_act', 'technology'], 'product', 0.0, yr) * unitc

    # 3 Sector results
    # 3.1 Transport
    tecs = list(set(msgSC.par('output', {'commodity': ['transport']}).technology))
    ...
    # 3.2 Industry
    tecs = list(set(msgSC.par('output', {'commodity': ['i_spec', 'i_therm']}).technology))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input')
    df = group(df, ['year_act', 'commodity'], 'product', 0.0, yr)
    df_hist = add_history(msgSC, tecs, nodeloc, df2, 'commodity')
    df = (df_hist + df).fillna(0)
    ...
    # 3.4 Residential and commercial
    tecs = list(set(msgSC.par('output',
                              {'commodity': ['rc_spec', 'rc_therm', 'non-comm']
                               }).technology))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input')
    df = group(df, ['year_act', 'technology'], 'product', 0.0, yr)
    df_hist = add_history(msgSC, tecs, nodeloc, df2, 'technology')
    ...
    tecs_imp = ['coal_imp', 'oil_imp', 'gas_imp', 'u5_imp', 'meth_imp',
                'loil_imp', 'LNG_imp', 'lh2_imp', 'foil_imp', 'eth_imp', 'elec_imp']
    ...
    df, df2 = model_output(msgSC, tecs, nodeloc, 'output')
    alldf['Useful energy ' + unitname] = plotdf_sec(
        plotyrs, df, ['year_act', 'technology'], 'product', 0.001, yr) * unitc

    # 4.3 Final energy
    tecs = list(set(msgSC.par('output', {'level': ['useful']}).technology))
    df, df2 = model_output(msgSC, tecs, nodeloc, 'input')
    alldf['Final energy ' + unitname] = plotdf_sec(
        plotyrs, df, ['year_act', 'technology'], 'product', 0.001, yr) * unitc
...
def plot(alldf, caseName, path):
    """Generate stacked bar charts and save to PDF + Excel."""
    path = Path(path)
    unitname = '(PJ)'
    ...
    pdf_path = path / 'plots' / f'{caseName}_{timestamp}-msgResults.pdf'
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        for subplt in [
            ...
            if alldf[subplt].empty:
                print(f'> WARNING: No solution data to plot for {subplt!r}')
                continue

            alldf[subplt] = alldf[subplt].loc[:, (alldf[subplt] > 0.01).any()]

            alldf[subplt].to_excel(
                writer_xls,
                sheet_name=subplt.replace('[', '(').replace(']', ')').replace('/', ''))

            fig = plt.figure(j)
            ax = fig.add_subplot(111)
            box = ax.get_position()

            if subplt == 'Power plant new capacity (GW)':
                dfs = alldf[subplt]
                ax.set_position([box.x0, box.y0, box.width * 0.8, box.height * 0.6])
            else:
                dfs = alldf[subplt].loc[
                    (alldf[subplt].index >= yr_min) & (alldf[subplt].index <= yr_max)]
            ...
            plt.grid(visible=True, which='both', linewidth=0.1,
                     color='silver', linestyle='-')
            ax.set_xticklabels(dfs.index, rotation=30)
            ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=fntsz)
